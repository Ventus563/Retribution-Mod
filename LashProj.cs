...
{
    public static class LashProj
    {
        public static Vector2 LashAI(Projectile projectile, float LashLength = 16, bool ignoreTiles = false, int sndgroup = 2, int sound = 39)
        {
            ...
            return AI_075(projectile, LashLength * speedIncrease / projectile.MaxUpdates, 
        ...
        public static void OnHitAny(Projectile projectile, bool crit, bool LashSoftSound)
        {
            OnHitAny(projectile, null, crit, LashSoftSound);
        }
        public static void OnHitAny(Projectile projectile, NPC target, bool crit, bool LashSoftSound)
        {
            if (crit) { Main.PlaySound(2, target.Center, LashSoftSound ? 98 : 40); }
            if (target != null)
            {

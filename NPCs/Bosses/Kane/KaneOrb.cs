...
namespace Retribution.NPCs.Bosses.Kane
{
    public class KaneOrb : ModNPC
    {
        ...
        private void BladeAttack()
        {
            ...
            Projectile.NewProjectile(npc.Center, Vector2.Zero, mod.ProjectileType("KaneBladeHostile"), damage, 6f, Main.myPlayer, npc.ai[1]);
        }
        ...
        private void SpawnPaladin()
        {
            ...
            {
                NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y + 32, mod.NPCType("tikitotem"));
            }
        }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life <= 0)
            {
                for (int x = 0; x < 50; x++)
                {
                    int dust = Dust.NewDust(npc.position, npc.width, npc.height, mod.DustType("Kane"));

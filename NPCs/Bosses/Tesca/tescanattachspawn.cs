	...
	public class tescanattachspawn : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (base.projectile.timeLeft == 180)
			{
				for (int i = 0; i < 15; i++)
				{
					int num = Dust.NewDust(base.projectile.Center, base.projectile.width, base.projectile.height, 185, (float)Main.rand.Next(-8, 8), (float)Main.rand.Next(-8, 8), 0, default(Color), 1f);
			...
			if (base.projectile.timeLeft < 60)
			{
				if (base.projectile.ai[1] > 10f)
				{
					...
					while ((float)num3 < num2)
					{
						...
						int num4 = Dust.NewDust(base.projectile.Center, 0, 0, 185, 0f, 0f, 0, default(Color), 1f);
			...
			else if (base.projectile.timeLeft < 90)
			{
				if (base.projectile.ai[1] > 20f)
				{
					...
					while ((float)num6 < num5)
					{
						...
						int num7 = Dust.NewDust(base.projectile.Center, 0, 0, 185, 0f, 0f, 0, default(Color), 1f);
			...
			else if (base.projectile.timeLeft < 120 && base.projectile.ai[1] > 30f)
			{
				float num8 = 15f;
				int num9 = 0;
				while ((float)num9 < num8)
				{
					...
					int num10 = Dust.NewDust(base.projectile.Center, 0, 0, 185, 0f, 0f, 0, default(Color), 1f);
			...
			{
				Main.PlaySound(SoundID.Item30, (int)base.projectile.position.X, (int)base.projectile.position.Y);
				Projectile.NewProjectile(base.projectile.Center.X, base.projectile.Center.Y - 10f, 0f, -7f, ModContent.ProjectileType<tescanattach>(), 25, 0f, 0, 0f, 0f);
			}
		}
	}
}

...
namespace Retribution.NPCs.Minibosses
{
	...
	public class sanguine : ModNPC
	{
		...
		{
			return spawnInfo.player.ZoneCrimson && Main.raining && NPC.downedSlimeKing && !NPC.AnyNPCs(ModContent.NPCType<sanguine>()) ? 0.01f : 0f;
		}
		...
		public override void AI()
		{
			...
			if (npc.ai[3] > 100f)
			{
				Projectile.NewProjectile(npc.position.X, npc.position.Y, shootToX, shootToY, ProjectileID.DeathLaser, 10, 0f, Main.myPlayer);
				npc.ai[3] = 0f;
			}
			...
			else if (AI_State == State_Fall)
			{
				if (npc.velocity.Y == 0)
				{
					NPC.NewNPC((int)npc.Bottom.X, (int)npc.Bottom.Y, NPCID.RedSlime);
					NPC.NewNPC((int)npc.Bottom.X + 25, (int)npc.Bottom.Y, NPCID.RedSlime);
					NPC.NewNPC((int)npc.Bottom.X - 25, (int)npc.Bottom.Y, NPCID.RedSlime);
					AI_State = State_Asleep;
					AI_Timer = 0;
				}
		...
		{
			RetributionWorld.downedSanguine = true;
		}
	}
}
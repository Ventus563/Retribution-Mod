...
namespace Retribution.NPCs.Minibosses
{
	[AutoloadBossHead]
	public class vilacious : ModNPC
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Vilacious");
		...
		{
			return spawnInfo.player.ZoneCorrupt && Main.raining && NPC.downedSlimeKing && !NPC.AnyNPCs(ModContent.NPCType<vilacious>()) ? 0.01f : 0f;
		}
		...
		public override void AI()
		{
			...
			float shootToX = target.Center.X + (float)target.width * 0.5f - npc.Center.X;
			float shootToY = target.Center.Y - npc.Center.Y;
			float distance = (float)System.Math.Sqrt((double)(shootToX * shootToX + shootToY * shootToY));

			if (npc.ai[3] > 100f)
			{
				Projectile.NewProjectile(npc.position.X, npc.position.Y, shootToX, shootToY, ProjectileID.EyeLaser, 10, 0f, Main.myPlayer);
				npc.ai[3] = 0f;
			}
			...
			else if (AI_State == State_Notice)
			{
				...
				if (Main.player[npc.target].Distance(npc.Center) < 700f)
				{
			...
			else if (AI_State == State_Fall)
			{
				if (npc.velocity.Y == 0)
				{
					NPC.NewNPC((int)npc.Bottom.X, (int)npc.Bottom.Y, NPCID.PurpleSlime);
					NPC.NewNPC((int)npc.Bottom.X + 25, (int)npc.Bottom.Y, NPCID.PurpleSlime);
					NPC.NewNPC((int)npc.Bottom.X - 25, (int)npc.Bottom.Y, NPCID.PurpleSlime);
					AI_State = State_Asleep;
					AI_Timer = 0;
				}
		...
		{
			RetributionWorld.downedVilacious = true;
		}
	}
}